...
class LogMelFilterBanks(nn.Module):
    def __init__(
        ...
        norm_mel: Optional[Literal["slaney"]] = None,
        mel_scale: Literal["slaney", "htk"] = "htk",
    ):
        ...
        self.f_max_hz = f_max_hz if f_max_hz else self.samplerate / 2
    ...
    def forward(self, x):
        ...
        tmp_spectrogram = self.spectrogram(x)

        power_stft = torch.pow(torch.abs(tmp_spectrogram), self.power).transpose(1, 2)
        return torch.log(torch.matmul(power_stft, mel_fbanks) + 1e-6).transpose(1, 2)
